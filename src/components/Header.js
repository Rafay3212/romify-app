...
function Header() {
    ...
    const navItems = [
        { icon: "https://cdn.builder.io/api/v1/image/assets/TEMP/39605f61eb60b9b227c7385613416ec71655b85e38f9fb9920b57feaf43a615c?placeholderIfAbsent=true&apiKey=f0392588affe43b6a23cab71c5f124c5", text: "Home", to: "/dashboard" },
                ...
                <nav className="flex flex-wrap gap-10 max-md:max-w-full">
                    <div className="flex flex-wrap flex-auto gap-7 items-end text-base font-bold text-black whitespace-nowrap max-md:max-w-full">
                        {navItems.map((item, index) => (
                            ...
                            >
                                <div className={`flex relative gap-2.5 justify-center items-center p-2.5 ${activeIndex === index ? 'text-sky-600' : ''}`}>
                                    <img loading="lazy" src={item.icon} alt={item.text} className="object-contain z-0 shrink-0 self-stretch my-auto w-5 aspect-[0.91]" />
